...
// --- Hardcoded seed data shaped for this page ---
const PROFILE_SEED = {
  fullName: "Ada Lovelace",
  email: "ada.lovelace@example.com",
  phone: "+1 (415) 555-0137",
  role: "Valued Customer",
  bio: "Prefers morning appointments; loves data-driven care.",
  avatarUrl: "", // leave empty to use the initials placeholder
  address: {
...
const normalizeMe = (api = {}) => ({
  fullName:
    `${api.first_name || ""} ${api.last_name || ""}`.trim() || "Unnamed User",
  email: api.email || "",
  phone: api.phone_number || "",
  role: api.role || "",
  bio: api.bio || "",
  avatarUrl: api.avatar || "",
  // The API has a single "address" string; your UI has separate fields,
  // so we map it to line1 for now.
  address: {
    line1: api.address || "",
    city: "",
    zip: "",
...
// Simple helper to render initials in the placeholder avatar
const initials = (name) =>
  (name || "")
    .trim()
    .split(/\s+/)
    ...
    <div className="font-sans bg-gray-100 min-h-screen">
      <div className="lg:pl-64 flex flex-col min-h-screen">
        ...
        <aside className="hidden lg:fixed lg:inset-y-0 lg:w-64 lg:flex lg:flex-col bg-[#083d41] p-4">
          ...
          <nav className="mt-8">{/* Nav links would go here */}</nav>
        </aside>
...
const ProfilePage = () => {
  ...
  // Load the logged-in user from the API
  ...
  useEffect(() => {
    ...
    // Pick the correct endpoint whether baseURL already ends with /api/v1 or not
    const base = authApiClient?.defaults?.baseURL || "";
    const path = /\/api\/v1\/?$/i.test(base)
      ? "/users/me/"
      : "/api/v1/users/me/";

    (async () => {
      try {
        const { data } = await authApiClient.get(path);
        if (!cancelled) setMe(normalizeMe(data));
      } catch (e) {
  ...
  const profile = useMemo(() => me ?? PROFILE_SEED, [me]);
        ...
        return (
          <form className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 rounded-lg bg-gray-50 border-gray-200 focus:border-green-300 focus:ring focus:ring-green-200 focus:ring-opacity-50"
                />
              </div>
              <div className="relative">
                <label className="block text-sm font-medium text-gray-700 mb-1">
            ...
            <div>
              ...
              <textarea
                rows="4"
                className="w-full px-4 py-3 rounded-lg bg-gray-50 border-gray-200 focus:border-green-300 focus:ring focus:ring-green-200 focus:ring-opacity-50"
          ...
          <div className="bg-white p-6 rounded-xl shadow-md text-center">
            <div className="relative inline-block mb-4">
              <img
                src={
                  profile.avatarUrl ||
                  `https://placehold.co/128x128/EBF4FA/333333?text=${encodeURIComponent(
