...
const DotsVerticalIcon = () => (
    <svg className="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 5v.01M12 12v.01M12 19v.01" /></svg>
);
...
const UserDashboardLayout = ({ children }) => {
    ...
    const SidebarContent = () => (
        <div className="flex flex-col h-full bg-[#083d41] text-white">
            ...
            <nav className="flex-grow p-4 space-y-2">
                {navItems.map(item => (
                    <a href="#" key={item.label} className={`flex items-center px-4 py-3 rounded-lg transition-colors ${item.active ? 'bg-green-500' : 'hover:bg-gray-700'}`}>
                        {item.icon}
    ...
    return (
        <div className="font-sans bg-gray-100 min-h-screen">
            ...
            <div className="hidden lg:fixed lg:inset-y-0 lg:w-64 lg:flex lg:flex-col"><SidebarContent /></div>
...
const MyBookingsPage = () => {
    ...
    const allBookings = [
        { id: '#1234', date: '2025-10-15', service: 'Deep House Clean', status: 'Upcoming', price: '$150', team: ['https://placehold.co/40x40/EBF4FA/333333?text=A', 'https://placehold.co/40x40/A0DEFF/333333?text=B'] },
        { id: '#1233', date: '2025-09-28', service: 'Office Cleaning', status: 'Completed', price: '$250', team: ['https://placehold.co/40x40/CAF4FF/333333?text=C'] },
        { id: '#1232', date: '2025-09-10', service: 'Kitchen Cleaning', status: 'Completed', price: '$80', team: ['https://placehold.co/40x40/EBF4FA/333333?text=A'] },
        { id: '#1231', date: '2025-08-22', service: 'Window Cleaning', status: 'Cancelled', price: '$120', team: [] },
        { id: '#1230', date: '2025-07-15', service: 'Deep House Clean', status: 'Completed', price: '$150', team: ['https://placehold.co/40x40/A0DEFF/333333?text=B', 'https://placehold.co/40x40/5AB2FF/333333?text=D'] },
    ];
    
    const filteredBookings = allBookings.filter(booking => filter === 'All' || booking.status === filter);
        ...
        <div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    {filters.map(f => (
                        <button 
                            key={f}
                            onClick={() => setFilter(f)}
                            className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors ${filter === f ? 'bg-green-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-200'}`}
                        >
                            {f}
                        </button>
                    ))}
            ...
            <div className="space-y-6">
                {filteredBookings.map(booking => (
                    <div key={booking.id} className="bg-white p-6 rounded-xl shadow-md transition-shadow hover:shadow-lg">
                        <div className="flex flex-wrap justify-between items-start gap-4">
                            ...
                            <div className="flex items-center gap-4">
                                <span className={`px-3 py-1 text-xs font-semibold rounded-full border ${getStatusClass(booking.status)}`}>
                                    {booking.status}
                                </span>
                                <span className="text-xl font-bold text-[#083d41]">{booking.price}</span>
                                <button className="text-gray-500 hover:text-gray-800"><DotsVerticalIcon/></button>
                            </div>
                        </div>
                                ...
                                <div className="flex items-center space-x-2">
                                     <div className="flex -space-x-2">
                                        {booking.team.map((memberImg, index) => (
                                            <img key={index} className="w-8 h-8 rounded-full border-2 border-white" src={memberImg} alt={`Team member ${index + 1}`} />
                                        ))}
                                    </div>
                                </div>
                            </div>
